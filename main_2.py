...
def _istft(y, n_fft, hop_length, win_length):
    return librosa.istft(stft_matrix=y, n_fft=n_fft, hop_length=hop_length, win_length=win_length)
...
def main():
    ...
    for i in indx:
        ...
        # _stft_result = stft_result.copy()
        # _stft_result[:, 30:] = 0

        signal_stft = _istft(stft_result, n_fft, hop_length, win_length)

        stft_db = _amp_to_db(stft_result)
        # stft_db[stft_db < 40] = 0
        _stft_db = _db_to_amp(stft_db)

        signal_stft = _istft(_stft_db, n_fft, hop_length, win_length)

        plt.plot(data, 'b')
        plt.plot(signal_stft, 'g')
        plt.show()
